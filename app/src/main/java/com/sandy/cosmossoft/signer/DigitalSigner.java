...
public class DigitalSigner {
    ...
    public DigitalSigner(String keyStoreFile, char[] keyStorePassword, Context context) {
        try {
            ...
            this.f154ks = instance;
            instance.load(context.getAssets().open(keyStoreFile), keyStorePassword);
            String nextElement = this.f154ks.aliases().nextElement();
            this.alias = nextElement;
            this.privateKey = (PrivateKey) this.f154ks.getKey(nextElement, keyStorePassword);
        } catch (Exception e) {
            Log.e("eror", "erro");
            e.printStackTrace();
        }
    ...
    private Document sign(Document xmlDoc) throws Exception {
        X509Certificate x509Cert = (X509Certificate) this.f154ks.getCertificate(this.alias);
        XMLSignature signature = new XMLSignature(xmlDoc, "", "http://www.w3.org/2000/09/xmldsig#rsa-sha1");
